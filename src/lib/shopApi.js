...
export const usersApi = {
  getByEmail: async (email) => {
    ...
    const lookupPromise = (async () => {
      ...
      // Fixed: Use 'profiles' table to sync with auth.js
      const supabaseResponse = await safeSupabase(() =>
  ...
  upsert: async (payload) => {
    ...
    if (existing?.[0]) {
      ...
      const insertResponse = await safeSupabase(() => supabase.from('profiles').insert([payload]).select());
  ...
  getByGoogleId: async (googleId) => {
    ...
    const lookupPromise = (async () => {
      ...
      const supabaseResponse = await safeSupabase(() =>
        supabase.from('profiles').select('*').eq('googleId', id).limit(1),
      );
      const data = supabaseResponse?.data;
      if (data?.[0]) {
        const record = data[0];
